...
// Bounded LRU cache for Hive schema (max 50 connections to prevent unbounded growth)
const HIVE_CACHE_MAX = 50;
const globalHiveCache = new Map();
const _hiveCacheGet = (id) => globalHiveCache.get(id);
const _hiveCacheSet = (id, val) => {
    if (globalHiveCache.size >= HIVE_CACHE_MAX && !globalHiveCache.has(id)) {
        globalHiveCache.delete(globalHiveCache.keys().next().value); // evict oldest
    }
    globalHiveCache.set(id, val);
};
...
router.post('/search/fetch', requireConnectionAccess('id'), async (req, res) => {
    ...
    try {
        ...
        if (conn.type === 'starrocks') { const connection = await getSrConnection(conn.host, conn.port, conn.username, conn.password); const [result] = await connection.query(`SELECT TABLE_SCHEMA as db, TABLE_NAME as tbl, COLUMN_NAME as col FROM information_schema.columns WHERE COLUMN_NAME LIKE ? LIMIT 100`, [`%${col}%`]); await connection.end(); res.json(result); } 
            ...
            if (!_hiveCacheGet(id)) _hiveCacheSet(id, {}); const myCache = _hiveCacheGet(id); let setup = await setupHiveClient(conn);
            try {
...
router.post('/explore/query', requireConnectionAccess('connection_id'), async (req, res) => {
    try {
        ...
        // Strip trailing semicolons and validate
        ...
        // Reject embedded semicolons (prevents multi-statement injection)
        if (/;/.test(safeQuery)) {
            return res.status(400).json({ error: "Multi-statement queries are not allowed." });
        }

        if (safeQuery.length > 50000) {
        ...
        try {
            ...
            if (conn.type === 'starrocks') {
                try {
                    ...
                    const escapedTableName = table_name.replace(/[\\%_]/g, '\\$&');
                    const [statusRes] = await connection.query(`SHOW TABLE STATUS FROM \`${db_name}\` LIKE ?`, [escapedTableName]);
