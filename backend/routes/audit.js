...
const getAirflowDb = async (connection_id) => {
    ...
    const sslConfig = process.env.AIRFLOW_DB_CA_CERT
        ? { ca: require('fs').readFileSync(process.env.AIRFLOW_DB_CA_CERT) }
...
/** Safely end an airflow client, suppressing close errors. */
...
router.post('/master-dags', requireConnectionAccess('connection_id'), async (req, res) => {
    ...
    let airflowDb;
    try {
        const result = await getAirflowDb(connection_id);
        airflowDb = result.airflowDb;
        const uiUrl = result.uiUrl;
        ...
        if (airflowDb) await safeEnd(airflowDb);
    }
});
...
router.post('/child-dags', requireConnectionAccess('connection_id'), async (req, res) => {
    ...
    let airflowDb;
    try {
        ({ airflowDb } = await getAirflowDb(connection_id));
...
router.post('/tasks', requireConnectionAccess('connection_id'), async (req, res) => {
    ...
    try {
        ...
        console.error('[Audit tasks error]:', e);
