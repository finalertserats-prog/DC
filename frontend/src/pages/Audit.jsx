...
const StatusBadge = ({ state }) => {
    if (!state) return <span className="text-muted-foreground/70 text-[11px]">—</span>;
    const map = {
    ...
    return <span className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-widest uppercase border ${map[state] || 'bg-muted/50 text-muted-foreground border-border'}`}>{state}</span>;
};

const StateBadge = ({ isPaused }) => {
    if (isPaused === null || isPaused === undefined) return <span className="text-muted-foreground/70 text-[11px]">—</span>;
    return isPaused
        ? <span className="px-2 py-0.5 rounded text-[10px] font-bold tracking-widest uppercase border bg-muted/50 text-muted-foreground border-border">Paused</span>
        ...
        <tr className={`bg-indigo-50/30 ${isLast ? '' : 'border-b border-indigo-100/60'}`}>
            <td className="pl-14 pr-4 py-3 whitespace-nowrap">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-semibold text-foreground/70">{child.dag_id}</span>
                    {child.is_task_only && <span className="text-[9px] bg-muted text-muted-foreground/70 border border-border px-1.5 py-0.5 rounded uppercase tracking-wider font-bold">task</span>}
                </div>
            </td>
            <td className="px-4 py-3 whitespace-nowrap"><StateBadge isPaused={child.is_paused} /></td>
            <td className="px-4 py-3 whitespace-nowrap text-[11px] text-muted-foreground/70 font-mono">
            ...
            <td className="px-4 py-3 whitespace-nowrap text-[12px] text-muted-foreground">{child.avg_run_minutes != null ? `${child.avg_run_minutes} min` : '—'}</td>
...
function MasterRow({ dag, uiUrl, connectionId, searchQuery }) {
    ...
    return (
        <>
            <tr className="hover:bg-muted/50/60 transition-colors border-b border-border/50">
                <td className="px-5 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                        <button onClick={toggle} className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/70 hover:text-indigo-600 hover:bg-indigo-50 transition-all">
                            {loadingChildren
                        ...
                        <span className="text-[13px] font-bold text-foreground">{dag.dag_id}</span>
                    </div>
                </td>
                <td className="px-4 py-4 whitespace-nowrap"><StateBadge isPaused={dag.is_paused} /></td>
                <td className="px-4 py-4 whitespace-nowrap">
                    <span className="text-[11px] font-mono text-foreground/70 bg-muted/50 border border-border px-2 py-1 rounded">
                ...
                <td className="px-4 py-4 whitespace-nowrap text-[12px] text-foreground/70">{dag.avg_run_minutes != null ? `${dag.avg_run_minutes} min` : '—'}</td>
                <td className="px-4 py-4 whitespace-nowrap">
                    {uiUrl && (
                        <a href={`${uiUrl.replace(/\/$/, '')}/dags/${dag.dag_id}/grid`} target="_blank" rel="noreferrer"
                            className="bg-card border border-indigo-200 text-indigo-700 hover:bg-indigo-50 px-3 py-1.5 rounded text-[11px] font-bold flex items-center gap-1.5 transition-colors w-fit">
                            <i className="fas fa-external-link-alt text-[10px]"></i> Airflow
                        </a>
                    )}
                </td>
            ...
            {expanded && children !== null && children.length === 0 && (
                <tr className="bg-muted/50/40">
                    <td colSpan="8" className="pl-14 py-3 text-[12px] text-muted-foreground/70 italic">No triggered child DAGs found.</td>
                </tr>
            )}
        </>
    );
        ...
        <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden mb-6">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full">
                    <thead className="bg-muted/50/80 border-b border-border/50">
                        <tr>
                            <th className="px-5 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">DAG ID</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">State</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Schedule (IST)</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Yesterday Run</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Today's Run</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Next Run</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Avg Run</th>
                            <th className="px-4 py-3.5 text-left text-[11px] font-bold text-muted-foreground/70 uppercase tracking-wider">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading && dags.length === 0 ? (
                            <tr><td colSpan="8" className="py-14 text-center text-muted-foreground/70"><i className="fas fa-spinner fa-spin text-xl mb-2 text-indigo-400 block"></i>Loading...</td></tr>
                        ) : dags.length === 0 ? (
                            <tr><td colSpan="8" className="py-14 text-center text-muted-foreground/70 italic">{emptyMsg}</td></tr>
                        ) : dags.map(dag => (
...
export default function Audit() {
    ...
    return (
        <div className="flex-1 h-full overflow-y-auto bg-background custom-scrollbar fade-in">
            {/* ── Header ──────────────────────────────────────────────── */}
            <header className="bg-card px-8 py-5 flex justify-between items-center border-b border-border shrink-0 shadow-sm">
                <div>
                    <h1 className="text-[22px] font-bold text-foreground flex items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Master pipeline DAGs — yesterday &amp; today run status. Expand rows to see child DAGs.</p>
                </div>
                <div className="flex gap-3 items-center">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="border border-gray-300 rounded-md bg-card flex items-center px-4 py-2 shadow-sm text-sm text-indigo-600 hover:border-indigo-400 hover:bg-indigo-50 transition-all font-bold min-w-[200px] justify-between"
                        >
                            <span className="flex items-center"><i className="far fa-calendar-alt mr-2 text-indigo-400"></i>{timeRangeDisplay}</span>
                            <i className="fas fa-caret-down text-muted-foreground/70 ml-2"></i>
                        </button>
                        {isTimeRangeOpen && (
                            <>
                                <div className="fixed inset-0 z-40" onClick={() => setIsTimeRangeOpen(false)}></div>
                                <div className="absolute top-full right-0 mt-2 w-[480px] bg-card rounded-xl shadow-2xl border border-border z-50 overflow-hidden flex flex-col fade-in">
                                    <div className="px-5 py-3 border-b border-border/50 flex items-center gap-3 bg-muted/50/50 shrink-0">
                                        <i className="fas fa-pencil-alt text-muted-foreground/70 text-sm"></i>
                                        <span className="text-[15px] font-bold text-foreground/80">Edit time range</span>
                                    </div>
                                    <div className="p-5 flex gap-5">
                                        <div className="w-1/3">
                                            <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-2 block">Range Type</label>
                                            <div className="space-y-1">
                                                {['no_filter','last','specific','custom'].map(type => (
                                                    <div key={type} onClick={() => setRangeType(type)}
                                                        className={`px-3 py-2 text-sm rounded-lg cursor-pointer transition-colors ${rangeType === type ? 'bg-indigo-50 text-indigo-700 font-bold border border-indigo-100' : 'text-foreground/70 hover:bg-muted/50 border border-transparent'}`}>
                                        ...
                                        <div className="w-2/3 border-l border-border/50 pl-5 min-h-[140px]">
                                            {rangeType === 'no_filter' && (
                                                <div className="text-sm text-muted-foreground italic mt-6 flex flex-col items-center">
                                                    <i className="far fa-calendar-check text-4xl text-indigo-200 mb-3"></i>All time data will be included.
                                                </div>
                                            )}
                                            {rangeType === 'last' && (
                                                <div>
                                                    <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-2 block">Duration</label>
                                                    <div className="flex gap-2">
                                                        ...
                                                        <select value={lastUnit} onChange={e => setLastUnit(e.target.value)}
                                                            className="w-2/3 border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:border-indigo-500 bg-card cursor-pointer">
                                            ...
                                            {rangeType === 'specific' && (
                                                <div>
                                                    <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-2 block">Select Date</label>
                                                    <input type="date" value={specificDate} onChange={e => setSpecificDate(e.target.value)}
                                                        className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:border-indigo-500 text-foreground/80 cursor-pointer" />
                                                </div>
                                            )}
                                            {rangeType === 'custom' && (
                                                <div className="space-y-3">
                                                    <div>
                                                        <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-1.5 block">Start Date</label>
                                                        <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)}
                                                            className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:border-indigo-500 text-foreground/80 cursor-pointer" />
                                                    </div>
                                                    <div>
                                                        <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-1.5 block">End Date</label>
                                    ...
                                    <div className="px-5 py-3 bg-muted/50 border-t border-border/50 flex justify-end gap-3 shrink-0 rounded-b-xl">
                                        <button onClick={() => setIsTimeRangeOpen(false)} className="px-5 py-2 rounded-lg text-xs font-bold text-muted-foreground hover:bg-muted/80 transition-colors uppercase tracking-wide">CANCEL</button>
                    ...
                    <select value={selConn || ''} onChange={e => setSelConn(e.target.value)}
                        className="border border-gray-300 rounded-md bg-card text-foreground/80 px-3 py-2 text-sm focus:outline-none shadow-sm min-w-[150px] cursor-pointer">
                        {conns.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <button onClick={fetchMasterDags}
                        className="border border-gray-300 bg-card text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 px-4 py-2 rounded-md text-sm font-bold shadow-sm flex items-center gap-2 transition-all">
            ...
            <div className="p-8 max-w-[1600px] mx-auto">
                ...
                <div className="grid grid-cols-5 gap-4 mb-4">
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Total DAG Runs</h4>
                        <span className="text-4xl font-bold text-foreground">{formatK(stats.total)}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center relative overflow-hidden">
                        <div className="absolute top-2 right-2 text-gray-100"><i className="fas fa-sun text-5xl"></i></div>
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2 z-10">Total Runs Today</h4>
                        <span className="text-4xl font-bold text-foreground z-10">{stats.today_runs.toLocaleString()}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Total Task Count</h4>
                        <span className="text-4xl font-bold text-foreground">{formatK(stats.total_tasks)}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Avg Runtime (min)</h4>
                        <span className="text-4xl font-bold text-foreground">{Number(stats.avg_run_minutes).toFixed(4)}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Total Run Time (Hr)</h4>
                        <span className="text-4xl font-bold text-foreground">{Number(stats.total_run_hours).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                </div>
                <div className="grid grid-cols-5 gap-4 mb-8">
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-emerald-500 mb-2">Successful Runs</h4>
                        <span className="text-4xl font-bold text-foreground">{formatK(stats.success)}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-rose-500 mb-2">Failed Runs</h4>
                        <span className="text-4xl font-bold text-foreground">{stats.failed.toLocaleString()}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Success Rate %</h4>
                        <span className="text-4xl font-bold text-foreground">{successRate}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Failed Rate %</h4>
                        <span className="text-4xl font-bold text-foreground">{failRate}</span>
                    </div>
                    <div className="bg-card rounded-lg border border-border p-5 shadow-sm flex flex-col justify-center">
                        <h4 className="text-[12px] font-bold text-muted-foreground mb-2">Master DAGs</h4>
                        <span className="text-4xl font-bold text-foreground">{dags.length}</span>
                    </div>
                </div>
                ...
                <div className="mb-5">
                    <h2 className="text-[18px] font-extrabold text-foreground mb-3">SDP DAGs Audit</h2>
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <div className="flex gap-2">
                            {sectionTabs.map(tab => {
                                ...
                                const colorMap = {
                                    indigo:  { on: 'bg-indigo-600 text-white border-indigo-600',  badge: 'bg-indigo-500 text-white',   off: 'bg-card border-border text-foreground/70 hover:bg-muted/50' },
                                    emerald: { on: 'bg-emerald-600 text-white border-emerald-600', badge: 'bg-emerald-500 text-white',  off: 'bg-card border-border text-foreground/70 hover:bg-muted/50' },
                                    amber:   { on: 'bg-amber-500 text-white border-amber-500',     badge: 'bg-amber-400 text-white',    off: 'bg-card border-border text-foreground/70 hover:bg-muted/50' },
                                };
                                ...
                                return (
                                    <button key={tab.key} onClick={() => setActiveSection(tab.key)}
                                        ...
                                        {tab.label}
                                        <span className={`text-[10px] px-1.5 py-0.5 rounded-sm font-bold ${active ? c.badge : 'bg-muted text-muted-foreground'}`}>
                                            {tab.count}
                                        </span>
                                    </button>
                                );
                        ...
                        {/* Search */}
                        <div className="relative border border-gray-300 rounded-md bg-card flex items-center px-3 py-2 min-w-[280px] shadow-sm focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                            <i className="fas fa-search text-muted-foreground/70 mr-2 text-sm"></i>
                            <input type="text" placeholder="Search DAGs..." value={searchQuery}
                                onChange={e => setSearchQuery(e.target.value)}
                                className="text-sm text-foreground/80 focus:outline-none bg-transparent w-full" />
                            {searchQuery && (
                                <button onClick={() => setSearchQuery('')} className="text-muted-foreground/70 hover:text-foreground/70 ml-1">
                ...
                <div className="mt-2 text-[12px] text-muted-foreground/70 text-center">
