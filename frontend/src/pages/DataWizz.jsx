...
function Terminal({ logs, streaming, error, emptyText = 'Waiting...' }) {
    ...
    return (
        <div className="bg-neutral-900 dark:bg-neutral-950 rounded-2xl border border-border p-5 font-mono text-xs overflow-y-auto max-h-72 min-h-[120px] transition-colors duration-200">
            {!logs.length && !streaming && !error && (
                <p className="text-muted-foreground italic">{emptyText}</p>
            )}
            {logs.map((log, i) => (
                <div key={i} className="flex items-start gap-3 mb-2">
                    <span className="text-muted-foreground shrink-0 w-4">{log.step || '·'}</span>
        ...
        <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-200 font-bold text-sm
                ${active
                    ...
                    : done
                        ? 'text-foreground/70 hover:bg-card hover:text-emerald-600 cursor-pointer'
                        : 'text-muted-foreground/70 cursor-not-allowed opacity-50'}`}
        >
            <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-xs font-bold transition-all
                ${active ? 'bg-card/20 text-white' : done ? 'bg-emerald-100 text-emerald-600' : 'bg-muted text-muted-foreground/70'}`}>
...
function ChartCard({ chart }) {
    const VIZ = {
        ...
        table:                  { icon: 'fa-table',      color: 'text-foreground/70',    bg: 'bg-muted/50 border-border'       },
    ...
    const v = VIZ[chart.viz_type] || { icon: 'fa-chart-area', color: 'text-muted-foreground', bg: 'bg-muted/50 border-border' };
    const metrics = (chart.metrics || []).map(m => m.label || `${m.aggregate}(${m.column?.column_name})`);

    return (
        <div className="bg-card border border-border rounded-2xl p-4 hover:border-emerald-200 hover:shadow-sm transition-all">
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                        <h4 className="text-foreground text-sm font-bold truncate">{chart.title}</h4>
                        <span className="text-[9px] font-bold bg-muted text-muted-foreground/70 px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0">w{chart.width}</span>
                    </div>
                    <p className={`text-[10px] font-bold uppercase tracking-widest mb-1.5 ${v.color}`}>{chart.viz_type}</p>
                    {metrics.length > 0 && <p className="text-[11px] text-muted-foreground"><span className="text-foreground/70">Metrics:</span> {metrics.join(', ')}</p>}
                    {chart.groupby?.length > 0 && <p className="text-[11px] text-muted-foreground"><span className="text-foreground/70">Group by:</span> {chart.groupby.join(', ')}</p>}
                    {chart.reasoning && <p className="text-[10px] text-muted-foreground/70 mt-1.5 italic leading-relaxed">{chart.reasoning}</p>}
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div>
            <label className="block text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5">
                {label} {hint && <span className="normal-case font-normal text-muted-foreground/70">{hint}</span>}
            </label>
            {children}
        </div>
    );
...
const INPUT = "w-full bg-muted/50 border border-border text-foreground placeholder-muted-foreground px-4 py-2.5 rounded-xl focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all text-sm font-medium";
const SELECT = "w-full bg-muted/50 border border-border text-foreground px-4 py-2.5 rounded-xl focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all text-sm font-medium disabled:opacity-40 appearance-none cursor-pointer";
            ...
            <aside className="w-64 bg-muted/50 border-r border-border flex flex-col shrink-0">
                ...
                <div className="px-6 py-5 border-b border-border">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-extrabold text-foreground text-base leading-tight">Data Viz</p>
                            <p className="text-[10px] text-muted-foreground font-medium">AI Dashboard Builder</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-4 pt-4">
                    <button
                        onClick={() => navigate('/')}
                        className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:bg-card hover:text-foreground transition-all font-medium"
                    >
                ...
                <div className="px-4 pt-4 flex-1 overflow-y-auto">
                    <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest px-3 mb-2">Steps</p>
                ...
                <div className="px-4 pb-4 border-t border-border pt-4 mt-2">
                    <div className="bg-card rounded-xl border border-border p-3 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-foreground truncate">{user.username || user.name || 'User'}</p>
                            <p className="text-[10px] text-muted-foreground/70 truncate">{user.email || 'Logged in'}</p>
                        </div>
                        <button
                            onClick={handleLogout}
                            title="Sign out"
                            className="w-7 h-7 flex items-center justify-center rounded-lg text-muted-foreground/70 hover:bg-red-50 hover:text-red-500 transition-all"
                        >
                ...
                <header className="bg-card border-b border-border px-8 py-4 flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        {STEPS.map(s => (
                            <div
                                key={s.n}
                                className={`w-2 h-2 rounded-full transition-all ${step === s.n ? 'bg-emerald-500 w-4' : s.done ? 'bg-emerald-300' : 'bg-muted/80'}`}
                            />
                        ))}
                    </div>
                </header>
                ...
                <main className="flex-1 overflow-y-auto px-8 py-8 bg-background">
                    <div className="max-w-3xl mx-auto space-y-6">
                        ...
                        {step === 1 && (
                            <div className="space-y-6">
                                <div className="bg-card rounded-2xl shadow-sm border border-border p-6 space-y-5">
                                    <div>
                                        <h2 className="text-base font-bold text-foreground mb-1">Choose Connection</h2>
                                        <p className="text-sm text-muted-foreground">Pick the table you want to build a dashboard for.</p>
                                    </div>
                                    <div className="grid grid-cols-3 gap-4">
                                        <Field label="Connection">
                                            <div className="relative">
                                                <select value={selConn?.id || ''} onChange={e => handleConnChange(e.target.value)} className={SELECT}>
                                                    <option value="" className="bg-card text-foreground">— pick one —</option>
                                                    {connections.map(c => <option key={c.id} value={c.id} className="bg-card text-foreground">{c.name}</option>)}
                                                </select>
                                                <i className="fas fa-chevron-down absolute right-3 top-3.5 text-muted-foreground/70 text-xs pointer-events-none"></i>
                                            </div>
                                        </Field>
                                        <Field label="Database">
                                            <div className="relative">
                                                <select value={selDb} onChange={e => handleDbChange(e.target.value)} disabled={!selConn || loadingDbs} className={SELECT}>
                                                    <option value="" className="bg-card text-foreground">— pick one —</option>
                                                    {databases.map(db => <option key={db} value={db} className="bg-card text-foreground">{db}</option>)}
                                                </select>
                                                <i className="fas fa-chevron-down absolute right-3 top-3.5 text-muted-foreground/70 text-xs pointer-events-none"></i>
                                            </div>
                                        ...
                                        <Field label="Table">
                                            <div className="relative">
                                                <select value={selTable} onChange={e => handleTableChange(e.target.value)} disabled={!selDb || loadingTables} className={SELECT}>
                                                    <option value="" className="bg-card text-foreground">— pick one —</option>
                                                    {tables.map(t => <option key={t} value={t} className="bg-card text-foreground">{t}</option>)}
                                                </select>
                                                <i className="fas fa-chevron-down absolute right-3 top-3.5 text-muted-foreground/70 text-xs pointer-events-none"></i>
                                            </div>
                                                ...
                                                <div className="overflow-y-auto max-h-52">
                                                    <table className="min-w-full">
                                                        <thead className="bg-muted/50 sticky top-0">
                                                            <tr>
                                                                <th className="px-4 py-2.5 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Column</th>
                                                                <th className="px-4 py-2.5 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Type</th>
                                                                <th className="px-4 py-2.5 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Description</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody className="bg-card">
                                                            {schemaInfo.columns?.map((col, i) => {
                                                                ...
                                                                }[col.type] || 'bg-muted/50 text-muted-foreground border-border';
                                                                return (
                                                                    <tr key={i} className="border-t border-border/50 hover:bg-muted/50 transition-colors">
                                                                        <td className="px-4 py-2 font-mono text-xs text-foreground/80">{col.column_name}</td>
                                                                        <td className="px-4 py-2">
                                                                            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${badge}`}>{col.type}</span>
                                                                        </td>
                                                                        <td className="px-4 py-2 text-[11px] text-muted-foreground/70 italic">{col.comment || '—'}</td>
                                                                    </tr>
                                                                );
                                                            })}
                                                        </tbody>
                                ...
                                <div className="flex justify-end">
                                    <button
                                        onClick={() => setStep(2)}
                                        disabled={!canStep2}
                                        className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-muted/80 disabled:text-muted-foreground/70 disabled:cursor-not-allowed text-white font-bold px-8 py-3 rounded-xl flex items-center gap-2 transition-all shadow-sm"
                                    >
                        ...
                        {step === 2 && (
                            <div className="space-y-6">
                                <div className="bg-card rounded-2xl shadow-sm border border-border p-6 space-y-5">
                                    <h3 className="text-sm font-bold text-foreground/80 border-b border-border/50 pb-3">Dashboard Settings</h3>
                                ...
                                <div className="bg-card rounded-2xl shadow-sm border border-border p-6 space-y-5">
                                    <h3 className="text-sm font-bold text-foreground/80 border-b border-border/50 pb-3">Superset Connection</h3>
                                    <div className="grid grid-cols-3 gap-4">
                                        <Field label="URL">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        {loadingDatasets && <span className="text-muted-foreground/70 text-xs"><i className="fas fa-spinner fa-spin mr-1"></i>Loading datasets…</span>}
                                    </div>
                                </div>

                                <div className="flex justify-between">
                                    <button onClick={() => setStep(1)} className="text-muted-foreground hover:text-foreground border border-border hover:border-gray-300 bg-card font-bold px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all text-sm">
                        ...
                        {step === 3 && (
                            <div className="space-y-6">
                                <div className="bg-card rounded-2xl shadow-sm border border-border p-6 space-y-5">
                                    <div>
                                        <h2 className="text-base font-bold text-foreground mb-1">AI Dashboard Plan</h2>
                                        <p className="text-sm text-muted-foreground">Two agents are analyzing your requirements and designing the optimal charts.</p>
                                    </div>
                                ...
                                {dashboardPlan && (
                                    <div className="space-y-4">
                                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-start gap-4">
                                            ...
                                            <div>
                                                <p className="font-extrabold text-foreground text-base">{dashboardPlan.dashboard_title}</p>
                                                <p className="text-muted-foreground text-sm mt-0.5">{dashboardPlan.reasoning}</p>
                                            </div>
                                        </div>
                                        ...
                                        {dashboardPlan.filters?.length > 0 && (
                                            <div className="bg-card rounded-2xl shadow-sm border border-border p-5">
                                                <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-3">{dashboardPlan.filters.length} filters</p>
                                ...
                                <div className="flex justify-between">
                                    <button onClick={() => setStep(2)} className="text-muted-foreground hover:text-foreground border border-border hover:border-gray-300 bg-card font-bold px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all text-sm">
                                        <i className="fas fa-arrow-left text-xs"></i> Back
                                    </button>
                                    <div className="flex gap-3">
                                        <button onClick={runPlan} disabled={planSSE.isStreaming} className="text-muted-foreground hover:text-foreground border border-border hover:border-gray-300 bg-card font-bold px-5 py-2.5 rounded-xl flex items-center gap-2 transition-all text-sm disabled:opacity-30">
                        ...
                        {step === 4 && (
                            <div className="space-y-6">
                                <div className="bg-card rounded-2xl shadow-sm border border-border p-6 space-y-5">
                                    <div>
                                        <h2 className="text-base font-bold text-foreground mb-1">Building Dashboard</h2>
                                        <p className="text-sm text-muted-foreground">Creating charts and assembling your Superset dashboard.</p>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 flex items-start gap-4">
                                            ...
                                            <div className="flex-1">
                                                <p className="font-extrabold text-foreground text-lg">Dashboard Created!</p>
                                        ...
                                        {buildResult.chartActions?.length > 0 && (
                                            <div className="bg-card rounded-2xl shadow-sm border border-border p-5">
                                                <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-4">Charts built ({buildResult.chartActions.length})</p>
                                                <div className="space-y-2">
                                                    {buildResult.chartActions.map(([id, action], i) => (
                                                        <div key={i} className="flex items-center gap-3 text-xs">
                                                            ...
                                                            <span className="font-mono text-muted-foreground/70">#{id}</span>
                                                            {dashboardPlan?.charts?.[i] && <span className="text-foreground/70 font-medium">{dashboardPlan.charts[i].title}</span>}
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                ...
                                <div className="flex justify-between">
                                    <button onClick={() => setStep(3)} className="text-muted-foreground hover:text-foreground border border-border hover:border-gray-300 bg-card font-bold px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all text-sm">
                                    ...
                                    <button
                                        ...
                                        className="text-muted-foreground hover:text-foreground border border-border hover:border-gray-300 bg-card font-bold px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all text-sm"
                                    >
