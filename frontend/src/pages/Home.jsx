...
export default function Home() {
    ...
    return (
        <div className="flex-1 h-full overflow-y-auto bg-background custom-scrollbar fade-in relative">
            ...
            <div className="bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-700 px-8 py-14 pb-28 relative shrink-0">
                ...
                <div className="relative z-50 max-w-5xl mx-auto flex flex-col items-center text-center">
                    ...
                    <div className="w-full max-w-3xl relative shadow-2xl rounded-xl z-50 transition-all duration-300">
                        <i className="fas fa-search absolute left-4 top-1/2 transform -translate-y-1/2 text-muted-foreground/70 text-lg"></i>
                        <input 
                            type="text" 
                            value={searchQuery}
                            ...
                            className="w-full pl-12 pr-4 py-4 rounded-xl text-foreground focus:outline-none focus:ring-4 focus:ring-indigo-300/50 text-base font-medium shadow-sm transition-all"
                        />
                        <button className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-muted text-foreground/70 px-3 py-1.5 rounded-lg text-xs font-bold border border-border flex items-center gap-2 pointer-events-none">
                            <i className="fas fa-globe"></i> Global Search
                        </button>

                        {/* MODERATE SEARCH DROPDOWN */}
                        {showSearch && searchQuery.length >= 2 && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-card rounded-xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.3)] border border-border overflow-hidden flex flex-col text-left fade-in">
                                
                                {/* STICKY TOTAL RESULTS HEADER */}
                                {!isSearching && (
                                    <div className="bg-muted/50/95 backdrop-blur-sm border-b border-border/50 px-5 py-3 flex justify-between items-center sticky top-0 z-10 shrink-0">
                                        <span className="text-[10px] font-extrabold text-muted-foreground uppercase tracking-widest flex items-center"><i className="fas fa-list-ul mr-2"></i>Search Results</span>
                                        {searchResults.length > 0 ? (
                                            ...
                                            <span className="bg-muted/80 text-muted-foreground text-[11px] font-black px-2.5 py-1 rounded border border-gray-300 shadow-sm">0 Matches</span>
                                        )}
                                    </div>
                                )}

                                {/* HEIGHT RESTRAINED TO 500px */}
                                <div className="overflow-y-auto max-h-[500px] custom-scrollbar bg-card">
                                    {isSearching ? (
                                        ...
                                        searchResults.map((res, i) => (
                                            <div key={i} onClick={() => handleResultClick(res)} className="p-4 border-b border-gray-50 hover:bg-indigo-50/60 cursor-pointer flex justify-between items-center transition-colors group">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-[15px] font-bold text-foreground group-hover:text-indigo-700 transition-colors">{res.name}</p>
                                                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mt-1 flex items-center gap-2">
                                                            {res.type === 'table' ? <><span className="bg-card border border-border px-1.5 py-0.5 rounded shadow-sm text-foreground/70">DB: {res.db}</span></> : ''} 
                                                            <span className="flex items-center gap-1"><i className="fas fa-server text-muted-foreground/50"></i> {res.connectionName}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                                <i className="fas fa-arrow-right text-muted-foreground/50 text-sm group-hover:text-indigo-500 group-hover:translate-x-1 transition-all"></i>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="p-8 text-center text-muted-foreground/70 text-sm italic">No assets found matching "{searchQuery}"</div>
                                    )}
                                </div>
                            </div>
                        )}
            ...
            <div className="max-w-[1600px] mx-auto px-8 -mt-12 relative z-20 pb-12">
                {loading ? (
                    <div className="flex justify-center p-12 bg-card rounded-xl shadow-sm border border-border/50"><i className="fas fa-circle-notch fa-spin text-3xl text-indigo-500"></i></div>
                ) : (
                    <>
                        <div className="flex justify-between items-end mb-4 px-2">
                            <h2 className="text-xl font-bold text-white drop-shadow-md">System Overview</h2>
                            <div className="bg-card p-1 rounded-lg shadow-sm flex items-center border border-border">
                                <i className="fas fa-filter text-muted-foreground/70 ml-3 mr-2"></i>
                                <select
                                    value={selectedServer}
                                    onChange={(e) => setSelectedServer(e.target.value)}
                                    className="bg-transparent text-foreground/80 text-sm font-bold focus:outline-none block px-2 py-1.5 cursor-pointer"
                                >
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
                            <div className="bg-card rounded-xl shadow-sm border border-border/50 flex flex-col h-96">
                                <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center">
                                    <h3 className="font-bold text-foreground/80 text-sm flex items-center gap-2"><i className="fas fa-list-alt text-muted-foreground/70"></i> Activity Feed</h3>
                                </div>
                                <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                                    {stats.activity.length === 0 ? (
                                        <div className="text-center text-muted-foreground/70 text-sm italic mt-10">No recent activity.</div>
                                    ) : stats.activity.map((act, i) => (
                                        <div key={i} className="flex gap-3 relative">
                                            {i !== stats.activity.length -1 && <div className="absolute left-[15px] top-8 bottom-[-24px] w-px bg-muted"></div>}
                                            ...
                                            <div>
                                                <p className="text-xs text-foreground"><span className="font-bold">admin</span> updated metadata for <span className="font-mono text-indigo-600 bg-indigo-50 px-1 rounded">{act.table_name}</span></p>
                                                <p className="text-[10px] text-muted-foreground/70 mt-0.5">{new Date(act.last_updated).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="bg-card rounded-xl shadow-sm border border-border/50 flex flex-col h-96">
                                <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center">
                                    <h3 className="font-bold text-foreground/80 text-sm flex items-center gap-2"><i className="fas fa-wind text-muted-foreground/70"></i> Airflow Activity</h3>
                                </div>
                                <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar">
                                    {!stats.airflowActivity || stats.airflowActivity.length === 0 ? (
                                        <div className="text-center text-muted-foreground/70 text-sm italic mt-10">No recent pipeline runs.</div>
                                    ) : stats.airflowActivity.map((run, i) => (
                                        <div key={i} className="flex justify-between items-center bg-muted/50/80 p-3 rounded-lg border border-border/50 hover:border-indigo-200 transition-colors cursor-pointer" onClick={() => navigate('/audit', { state: { selConn: {id: run.connection_name} , searchQuery: run.dag_id } })}>
                                            <div className="min-w-0 pr-3">
                                                <p className="text-xs font-bold text-foreground truncate" title={run.dag_id}>{run.dag_id}</p>
                                                <p className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-1.5"><i className="fas fa-server text-muted-foreground/50"></i> {run.connection_name} • {formatTimeAgo(run.start_date)}</p>
                                            </div>
                            ...
                            <div className="bg-card rounded-xl shadow-sm border border-border/50 flex flex-col h-96">
                                <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center">
                                    <h3 className="font-bold text-foreground/80 text-sm flex items-center gap-2"><i className="fas fa-database text-muted-foreground/70"></i> Data Assets Breakdown</h3>
                                </div>
                                <div className="flex-1 flex items-center justify-center gap-10 p-5">
                                    {selectedServer !== 'all' && filteredStats.type === 'hive' ? null : (
                                        <div className="flex flex-col items-center fade-in">
                                            ...
                                            <span className="font-bold text-foreground/80 text-sm">StarRocks</span>
                                    ...
                                    {selectedServer !== 'all' && filteredStats.type === 'starrocks' ? null : (
                                        <div className="flex flex-col items-center fade-in">
                                            ...
                                            <span className="font-bold text-foreground/80 text-sm">Hive / Hudi</span>
                            ...
                            <div className="bg-card rounded-xl shadow-sm border border-border/50 flex flex-col h-96">
                                <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center">
                                    <h3 className="font-bold text-foreground/80 text-sm flex items-center gap-2"><i className="fas fa-chart-pie text-muted-foreground/70"></i> Total Discovered Tables</h3>
                                </div>
                                <div className="flex-1 flex flex-col items-center justify-center p-5">
                                    <div className="relative w-44 h-44 rounded-full flex items-center justify-center mb-4 shadow-sm" style={{ background: `conic-gradient(#4f46e5 ${Math.min((filteredStats.total / (stats.totalAssets + 10 || 1)) * 100, 100)}%, #e0e7ff 0)` }}>
                                        <div className="w-32 h-32 bg-card rounded-full flex items-center justify-center flex-col shadow-inner">
                                            <span className="text-3xl font-black text-foreground">{filteredStats.total.toLocaleString()}</span>
                                            <span className="text-[10px] text-muted-foreground/70 font-bold uppercase tracking-wider">Tables</span>
                                        </div>
                                    </div>
