        ...
        <div className="flex-1 h-full flex flex-col fade-in bg-background">
            <header className="bg-card px-8 flex justify-between items-center z-10 h-20 border-b border-border shrink-0">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 font-medium">Manage user roles and connection access.</p>
                </div>
            ...
            <div className="p-8 h-full overflow-y-auto custom-scrollbar">
                {loading ? (
                    ...
                    <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
                            ...
                            <tbody className="bg-card divide-y divide-gray-100">
                                {users.map(u => (
                                    <tr key={u.id} className="hover:bg-indigo-50/30 transition-colors">
                                        ...
                                        {/* Role */}
                                        <td className="px-6 py-4">
                                            <span className={`text-[10px] font-semibold uppercase tracking-widest px-2 py-1 rounded ${u.role === 'admin' ? 'bg-indigo-100 text-indigo-700' : 'bg-muted text-muted-foreground'}`}>{u.role}</span>
                                        </td>
                                        {/* Access summary */}
                                        <td className="px-6 py-4">
                                            {u.role === 'admin' ? (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                {u.username !== currentUser.username ? (
                                                    ...
                                                    <span className="text-xs text-muted-foreground/70 italic">Cannot modify own account</span>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        {users.length === 0 && (
                            <div className="py-12 text-center text-muted-foreground italic text-sm">No users found.</div>
                        )}
                    </div>
                )}
            </div>
            ...
            {selectedUser && (
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center fade-in" onClick={(e) => { if (e.target === e.currentTarget) setSelectedUser(null); }}>
                    <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg flex flex-col border border-border/50 max-h-[80vh]">
                        {/* Header */}
                        <div className="px-6 py-4 border-b border-border/50 flex justify-between items-center bg-muted/50/80 shrink-0">
                            <div>
                                <h3 className="text-lg font-bold text-foreground">Connection Access</h3>
                                <p className="text-xs text-muted-foreground mt-0.5">
                                    Managing access for <span className="font-semibold text-indigo-600">{selectedUser.username}</span>
                                </p>
                            </div>
                            <button onClick={() => setSelectedUser(null)} className="text-muted-foreground/70 hover:text-foreground/70 transition-colors">
                                <i className="fas fa-times text-lg"></i>
                            </button>
                        </div>

                        {/* Quick actions */}
                        <div className="px-6 py-3 border-b border-border/50 flex gap-2 shrink-0">
                            ...
                            <span className="ml-auto text-xs text-muted-foreground/70 self-center">
                        ...
                        <div className="overflow-y-auto flex-1 custom-scrollbar">
                            {permLoading ? (
                                ...
                                <div className="py-12 text-center text-muted-foreground/70 italic text-sm">No connections exist yet.</div>
                            ) : (
                                <div className="divide-y divide-gray-100">
                                    {allConns.map(conn => {
                                        ...
                                        return (
                                            <div key={conn.id} className={`flex items-center gap-4 px-6 py-4 transition-colors ${granted ? 'bg-emerald-50/40' : 'hover:bg-muted/50'}`}>
                                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${TYPE_COLOR[conn.type] || 'bg-muted/50 text-muted-foreground'} border border-white shadow-sm`}>
                                                    <i className={`fas ${TYPE_ICON[conn.type] || 'fa-plug'} text-sm`}></i>
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-semibold text-foreground text-sm truncate">{conn.name}</p>
                                                    <p className="text-[11px] text-muted-foreground/70 font-mono truncate">{conn.masked_host || conn.host}:{conn.port}</p>
                                                </div>
                                                <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${granted ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground/70'}`}>
                                                ...
                                                <button
                                                    ...
                                                    className={`w-11 h-6 rounded-full transition-all duration-300 relative shrink-0 disabled:opacity-50 ${granted ? 'bg-emerald-500' : 'bg-muted/80'}`}
                                                ...
                                                >
                                                    {busy ? (
                                                        ...
                                                        <span className={`absolute top-0.5 w-5 h-5 bg-card rounded-full shadow transition-all duration-300 ${granted ? 'left-5' : 'left-0.5'}`}></span>
                                                    )}
                                                </button>
                                            </div>
                                        );
                        ...
                        {/* Footer */}
                        <div className="px-6 py-4 border-t border-border/50 shrink-0 text-right">
